...
// Guesses test/live mode purely from the shape of a stored key — used
// wherever the caller only has the key string and not the `mode`
// column (e.g. computing it once for freshly-saved credentials).
...
// Raw pro_integrations row for one user+provider, or null if they've
// never connected it (or have since disconnected — `connected: false`
// counts as "not there" for every caller in this file).
